...
import math
"""
N4.1 CORE外包固定TAPE圈數大于1TS時,如產品尺寸為20以下(含20)TAPE錯邊不可超
過1mm.如產品尺寸在20以上,則TAPE錯邊不可大于2mm.
"""
max_value = 255
max_value_H = 360 // 2
low_H = 0
...
while True:
    #original = cv.imread('./venv/4_2/PXL_20210226_065144783.jpg')
    #original = cv.imread('./venv/4_2/PXL_20210226_065205969.jpg')
    ...
    frame_threshold = cv.inRange(frame_HSV, (30, 0, 190), (60, 255, 255))
    ...
    #cv.drawContours(frame, [max_box], 0, (0, 255, 0), 1)
    ...
    image_threshold = cv.inRange(image_HSV, (0, 0, 0), (70, 255, 255))
    ...
    for x1, y1, x2, y2 in lines:
        ...
        #print(max_x-min_x,abs(max_box[2][0]-min_x))
        if(abs(y2-y1)/abs(x2-x1) < 1):
            print(y1, y2, min_y1, max_y1)
        if ( abs(y2-y1)/abs(x2-x1) < 1  and ((min_y1+2<y1<min_y1+12 and min_y1+2<y2<min_y1+12) or (max_y1-2>y1>max_y1-12 and max_y1-2>y2>max_y1-12))):
            try:
                #print( y1, y2,min_y1,max_y1)
