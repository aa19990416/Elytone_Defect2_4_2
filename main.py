...
while True:
    ...
    frame_threshold = cv.inRange(frame_HSV, (30, 0, 191), (55, 255, 255))
    ...
    print(max_box[0], max_box[1], max_box[2], max_box[3])
    image = frame[min_y:max_y, min_x:max_x]
    points = np.array([[min_x+25, max_y-25], [min_x+25, min_y+25], [max_x-25, min_y+25], [max_x-25, max_y-25]])
    black = (0, 0, 0)  # BGR
    ...
    canny = cv.Canny(blurred, 30, 40, 3, 3, True)
    ...
    for x1, y1, x2, y2 in lines:
        ...
        print(0, max_x-min_x)
        if ( (5<(x1+x2)/2<20 ) or (max_x-min_x-20<x1<max_x-min_x-5 and max_x-min_x-20<x2<max_x-min_x-5) ):
